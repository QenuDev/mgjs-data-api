 ...
 * Les frames de l'atlas capturé, telles que le paquet les lit — la même source que le composeur.
 *
 * Le `sourcePixelRatio` est passé, parce que `frameBox` divise la taille de source par lui et parce
 * que le composeur le passe aussi (`artBridge.js` → `frameEntry`) : une frame 2x de l'atlas se dessine
 * à la moitié de sa taille de source. Le laisser tomber ici faisait lire à ce fichier le double de ce
 * que le composeur dessine, et le désaccord ne se voyait pas tant que la fixture ne portait aucun
 * ratio — ce qu'elle ne faisait pas avant que l'item 26 ne le lui rende.
 */
async function fixtureFrames() {
  ...
  return new Map(
    Object.entries(sprites.frames).map(([key, meta]) => [
      key,
      frameBox({
